...
for data in df:
    data = data.drop(['Pos', 'Age', 'G', 'GS', 'MP', 'Tm', 'ORB', 'DRB', 'PF'], axis=1)
    ...
    for player in data['Player'].unique():
        ...
        # Assuming you're predicting the 'PTS' (points) for next season
        ...
        # Add the model for each player into the dictionary
        player_models[player] = model

    # Predicting points for next season for each player and storing them
